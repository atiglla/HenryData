...
CHANGE `id` `IdFecha` INTEGER NOT NULL;
ALTER TABLE `cliente` 
CHANGE `ID` `IdCliente` INTEGER NOT NULL;
ALTER TABLE `empleado` 
CHANGE `IDEmpleado` `IdEmpleado` INT(11) NULL DEFAULT NULL;
ALTER TABLE `proveedor` 
CHANGE `IDProveedor` `IdProveedor` INT(11) NULL DEFAULT NULL;
ALTER TABLE `sucursal` 
CHANGE `ID` `IdSucursal` INT(11) NULL DEFAULT NULL;
ALTER TABLE `tipo_gasto` 
CHANGE `Descripcion` `Tipo_Gasto` VARCHAR(100) CHARACTER SET utf8 COLLATE utf8_spanish_ci NOT NULL;
ALTER TABLE `producto` 
CHANGE `IDProducto` `IdProducto` INT(11) NULL DEFAULT NULL;
ALTER TABLE `producto`
...
ALTER TABLE `venta` 
CHANGE `Precio` `Precio` DECIMAL(15,3) NOT NULL DEFAULT '0';
...
UPDATE venta 
SET Cantidad = REPLACE(Cantidad, '\r', '');

INSERT INTO aux_venta 
			(IdVenta, Fecha, Fecha_Entrega, IdCliente, IdSucursal, 
             IdEmpleado, IdProducto, Precio, Cantidad, Motivo)
...
ALTER TABLE `empleado`
ADD `IdSucursal` INT NULL DEFAULT '0' AFTER `Sucursal`;
...
ALTER TABLE `producto` 
ADD `IdTipoProducto` INT NOT NULL DEFAULT '0' AFTER `Precio`;
...
FROM proveedor 
where Ciudad = 'Avellaneda'
ORDER BY 2, 1;

INSERT INTO aux_localidad 
           (Localidad_Original, Provincia_Original, Localidad_Normalizada, Provincia_Normalizada, IdLocalidad)
...
UPDATE `aux_localidad` 
SET Localidad_Normalizada = 'Capital Federal'
...
UPDATE `aux_localidad` 
SET Localidad_Normalizada = 'Córdoba'
...
/*Discretización*/
ALTER TABLE `cliente` 
	ADD `Rango_Etario` VARCHAR(20) NOT NULL DEFAULT '-' AFTER `Edad`;

UPDATE cliente SET Rango_Etario = '1_Hasta 30 años' 
WHERE Edad <= 30;
